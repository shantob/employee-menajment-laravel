                ...
                <div class="">
                    <form action="{{route('expenses.store')}}" method="post" class="p-3">
                        ...
                        <select class="form-select form-control" id="employee" name="expense_type_id">
                            <option selected disabled>Select Expense Type Name</option>
                            @foreach($expenses_types as $expenses_type)
                            <option value="{{$expenses_type->id}}">{{$expenses_type->name}}</option>
                            @endforeach
                        </select>
                        ...
                        <div class="row mt-4">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control" id="amount" name="amount" value="{{old('ammount')}}">
                                @error('amount')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <button type="submit" class="btn btn-success mt-3">Add Now</button>
                    </form>
                </div>
            </div>
        </div>
